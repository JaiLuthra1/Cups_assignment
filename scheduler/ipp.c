	...
	mime_type_t     *filetype)	/* I - First print file type, if any */
{
  ...
  /* Not the Source... Custom Log */
  cupsdLogMessage(CUPSD_LOG_WARN, "add_job(%p[%d], %p(%s), %p(%s/%s))",
                  con, con->number, printer, printer->name,
		  filetype, filetype ? filetype->super : "none",
		  filetype ? filetype->type : "none");

  cupsdLogMessage(CUPSD_LOG_WARN, "Custom Log Added: USER-JAI generates a warning");
  cupsdLogMessage(CUPSD_LOG_ERROR, "Custom Log Added: USER-JAI it's error");
